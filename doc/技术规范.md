# 文件管理系统 - 技术规范

## 📋 概述

本规范定义了使用 **Rust + Tauri + TypeScript** 技术栈开发文件管理系统的编码标准、命名约定、文件组织和最佳实践。

## ⚠️ 重要规则（必须遵守）

### 类型定义规范
**所有 TypeScript 数据结构的定义必须放在 `src/types/` 目录下，禁止在组件、服务或其他文件中直接定义接口或类型。**

- ✅ **正确**：在 `src/types/tag.ts` 中定义 `Tag` 接口，然后在组件中导入使用
- ❌ **错误**：在 `Toolbar.vue` 或其他组件中直接定义 `interface Tag`

详细说明请参考 [类型定义位置规范](#221-类型定义位置规范-⚠️-重要规则)

## 🎯 目标

1. 保持代码一致性，提高可读性和可维护性
2. 遵循 Rust 和 TypeScript 社区最佳实践
3. 确保团队协作效率
4. 便于代码审查和自动化检查

## 🏗️ 项目结构规范

### 1. 整体项目结构
```
file-manager/
├── src-tauri/                    # Rust后端代码
│   ├── src/
│   │   ├── main.rs              # 应用入口
│   │   ├── commands.rs          # Tauri命令定义
│   │   ├── models/              # 数据模型（领域模型）
│   │   ├── services/            # 业务服务
│   │   ├── database/            # 数据库层
│   │   ├── system/              # 系统接口层
│   │   └── utils/               # 工具函数
│   ├── Cargo.toml               # Rust依赖配置
│   ├── tauri.conf.json          # Tauri应用配置
│   └── build.rs                 # 构建脚本
├── src/                         # 前端代码（TypeScript/React）
│   ├── main.tsx                 # 应用入口
│   ├── App.tsx                  # 主组件
│   ├── components/              # 可复用组件
│   ├── pages/                   # 页面组件
│   ├── stores/                  # 状态管理
│   ├── services/                # 前端服务
│   ├── types/                   # TypeScript类型定义
│   ├── styles/                  # 样式文件
│   └── utils/                   # 工具函数
└── ...                          # 配置文件
```

### 2. 目录命名规范
- **小写字母**，使用连字符分隔：`src-tauri`, `database-migrations`
- **复数形式**：`models`, `services`, `components`
- **明确含义**：避免使用 `utils`, `helpers` 等模糊名称

## 🔤 命名规范

### 1. Rust 命名规范

#### 1.1 文件命名
- **蛇形命名法**（snake_case）：`file_service.rs`, `tag_repository.rs`
- **模块文件**：`mod.rs`（Rust 2018 风格使用目录名）

#### 1.2 类型命名
- **结构体/枚举**：大驼峰命名法（PascalCase）
  ```rust
  struct FileMetadata { ... }
  enum FileChangeType { ... }
  trait FileService { ... }
  ```

#### 1.3 变量和函数命名
- **变量/字段**：蛇形命名法（snake_case）
  ```rust
  let file_path: String;
  let is_active: bool;

  struct FileInfo {
      file_hash: String,
      current_path: String,
  }
  ```

- **函数/方法**：蛇形命名法（snake_case）
  ```rust
  fn calculate_file_hash(path: &str) -> Result<String>;
  async fn add_tag_to_file(file_path: &str, tag_name: &str) -> Result<bool>;
  ```

#### 1.4 常量命名
- **全大写**，下划线分隔
  ```rust
  const MAX_FILE_SIZE: u64 = 1024 * 1024 * 1024; // 1GB
  const DEFAULT_SCAN_INTERVAL: Duration = Duration::from_secs(300);
  ```

#### 1.5 生命周期参数
- **简短描述性**：`'a`, `'ctx`, `'db`
  ```rust
  struct FileCache<'a> {
      data: &'a [u8],
  }
  ```

### 2. TypeScript/React 命名规范

#### 2.1 文件命名
- **组件文件**：大驼峰命名法，`.tsx` 扩展名
  ```
  FileList.tsx
  TagPanel.tsx
  SearchBar.tsx
  ```

- **工具文件**：小驼峰命名法，`.ts` 扩展名
  ```
  fileService.ts
  tagStore.ts
  formatters.ts
  ```

#### 2.2 类型命名
- **接口/类型别名**：大驼峰命名法，`I` 前缀可选
  ```typescript
  interface FileInfo {
      path: string;
      size: number;
      type: string;
  }

  type SearchQuery = {
      tags: string[];
      operator: 'AND' | 'OR';
  };
  ```

#### 2.2.1 类型定义位置规范 ⚠️ **重要规则**
- **所有数据结构的定义必须放在 `src/types/` 目录下**
- **禁止在组件、服务或其他文件中直接定义接口或类型**
- **每个功能模块应有对应的类型文件**（如 `tag.ts`, `file.ts`）

**正确示例**：
```typescript
// ✅ 正确：在 src/types/tag.ts 中定义
export interface Tag {
  id: number;
  name: string;
  color: string | null;
}

// ✅ 正确：在组件中导入使用
import type { Tag } from '../types/tag';
```

**错误示例**：
```typescript
// ❌ 错误：在组件中直接定义类型
interface Tag {
  id: number;
  name: string;
}

// ❌ 错误：在服务文件中定义类型
export class TagService {
  interface Tag { ... }  // 错误！
}
```

**类型文件组织**：
```
src/types/
├── file.ts      # 文件相关类型
├── tag.ts       # 标签相关类型
├── api.ts       # API响应类型
└── index.ts     # 统一导出（可选）
```

#### 2.3 变量和函数命名
- **变量/常量**：小驼峰命名法
  ```typescript
  const fileList: FileInfo[] = [];
  let isLoading = false;

  // 常量使用全大写
  const MAX_RESULTS = 100;
  const DEFAULT_TAG_COLOR = '#3B82F6';
  ```

- **函数/方法**：小驼峰命名法
  ```typescript
  function calculateFileSize(bytes: number): string { ... }
  async function searchFiles(query: SearchQuery): Promise<SearchResult> { ... }
  ```

#### 2.4 组件命名
- **React组件**：大驼峰命名法
  ```typescript
  const FileList: React.FC<FileListProps> = ({ files }) => { ... };

  // 函数组件
  function TagPanel({ tags, onSelect }: TagPanelProps) { ... }
  ```

#### 2.5 CSS类名命名
- **BEM风格** 或 **Tailwind CSS**
  ```css
  /* BEM风格 */
  .file-list { ... }
  .file-list__item { ... }
  .file-list__item--selected { ... }

  /* Tailwind CSS（推荐） */
  <div className="flex flex-col gap-4 p-4 bg-white rounded-lg shadow">
  ```

## 📝 代码注释规范

### 1. Rust 注释规范

#### 1.1 文档注释（///）
- **公共API必须包含文档注释**
- 使用 Markdown 格式
- 包含示例代码

```rust
/// 计算文件的 SHA-256 哈希值
///
/// # 参数
/// - `path`: 文件路径
///
/// # 返回值
/// - `Ok(String)`: 十六进制格式的哈希值
/// - `Err(io::Error)`: 文件读取错误
///
/// # 示例
/// ```
/// let hash = calculate_file_hash("/path/to/file.txt")?;
/// println!("File hash: {}", hash);
/// ```
pub fn calculate_file_hash(path: &str) -> Result<String, std::io::Error> {
    // ...
}
```

#### 1.2 模块注释（//!）
```rust
//! 文件系统监控模块
//!
//! 提供实时文件变更检测功能，使用 `notify-rs` 库实现跨平台监控。
//! 支持创建、修改、删除、重命名等事件检测。

mod monitor;
mod event_handler;
```

#### 1.3 行内注释（//）
- **解释复杂逻辑**，而非描述代码
- **避免 obvious 注释**

```rust
// 增量哈希计算：每64KB读取一次，避免内存占用过大
let mut buffer = [0; 65536];

// 处理软删除：标记为不活跃，保留标签关联
file.is_active = false;
```

### 2. TypeScript 注释规范

#### 2.1 JSDoc 注释
- **公共函数和组件必须包含 JSDoc**
- 使用 `@param`, `@returns`, `@example` 标签

```typescript
/**
 * 根据文件大小返回可读的格式化字符串
 *
 * @param bytes - 文件大小（字节）
 * @returns 格式化后的文件大小字符串
 *
 * @example
 * ```typescript
 * formatFileSize(1024); // "1 KB"
 * formatFileSize(1048576); // "1 MB"
 * ```
 */
export function formatFileSize(bytes: number): string {
    // ...
}
```

#### 2.2 组件 Props 注释
```typescript
interface FileListProps {
    /** 要显示的文件列表 */
    files: FileInfo[];
    /** 文件选择回调函数 */
    onSelect?: (file: FileInfo) => void;
    /** 是否显示加载状态 */
    isLoading?: boolean;
}

/**
 * 文件列表组件
 *
 * 显示文件列表，支持选择、排序和搜索功能
 */
const FileList: React.FC<FileListProps> = ({
    files,
    onSelect,
    isLoading = false
}) => {
    // ...
};
```

#### 2.3 行内注释
```typescript
// 防抖处理：避免频繁搜索请求
const debouncedSearch = useDebounce(searchQuery, 300);

// TODO: 未来需要支持多选功能
// FIXME: 这里有一个内存泄漏问题需要修复
```

## 📁 文件组织规范

### 1. Rust 模块组织

#### 1.1 模块声明
```rust
// src/models/mod.rs
pub mod file;
pub mod tag;
pub mod file_tag;

// 重新导出常用类型
pub use file::File;
pub use tag::Tag;
pub use file_tag::FileTag;
```

#### 1.2 错误处理模块
```rust
// src/error.rs
use thiserror::Error;

#[derive(Error, Debug)]
pub enum AppError {
    #[error("文件不存在: {0}")]
    FileNotFound(String),

    #[error("数据库错误: {0}")]
    DatabaseError(#[from] diesel::result::Error),

    #[error("IO错误: {0}")]
    IoError(#[from] std::io::Error),
}

pub type Result<T> = std::result::Result<T, AppError>;
```

#### 1.3 服务层组织
```rust
// src/services/mod.rs
pub mod file_service;
pub mod tag_service;
pub mod search_service;
pub mod monitor_service;

// 服务 trait 定义
pub trait FileService: Send + Sync {
    async fn get_file_info(&self, path: &str) -> Result<FileInfo>;
    async fn add_tag_to_file(&self, path: &str, tag: &str) -> Result<bool>;
}

// 具体实现
pub struct FileServiceImpl {
    db_pool: DbPool,
    file_cache: Arc<FileCache>,
}
```

### 2. TypeScript/React 组件组织

#### 2.1 组件分类
```
src/components/
├── ui/              # 基础UI组件（按钮、输入框等）
│   ├── Button.tsx
│   ├── Input.tsx
│   └── Modal.tsx
├── file/            # 文件相关组件
│   ├── FileList.tsx
│   ├── FileItem.tsx
│   └── FilePreview.tsx
├── tag/             # 标签相关组件
│   ├── TagPanel.tsx
│   ├── TagItem.tsx
│   └── TagEditor.tsx
└── layout/          # 布局组件
    ├── Sidebar.tsx
    ├── Header.tsx
    └── MainLayout.tsx
```

#### 2.2 状态管理组织
```typescript
// src/stores/fileStore.ts
import { create } from 'zustand';

interface FileStore {
    files: FileInfo[];
    selectedFile: FileInfo | null;
    isLoading: boolean;

    // Actions
    setFiles: (files: FileInfo[]) => void;
    selectFile: (file: FileInfo | null) => void;
    addTagToFile: (filePath: string, tag: string) => Promise<void>;
}

export const useFileStore = create<FileStore>((set, get) => ({
    files: [],
    selectedFile: null,
    isLoading: false,

    setFiles: (files) => set({ files }),
    selectFile: (file) => set({ selectedFile: file }),

    addTagToFile: async (filePath, tag) => {
        set({ isLoading: true });
        try {
            await fileService.addTagToFile(filePath, tag);
            // 更新本地状态
        } finally {
            set({ isLoading: false });
        }
    },
}));
```

## 🔧 代码风格规范

### 1. Rust 代码风格

#### 1.1 格式化
- **使用 rustfmt**：`cargo fmt`
- **行长度**：不超过 100 个字符
- **缩进**：4个空格

#### 1.2 导入排序
```rust
// 标准库
use std::collections::HashMap;
use std::path::Path;

// 外部库
use diesel::prelude::*;
use serde::{Deserialize, Serialize};
use tokio::fs;

// 内部模块
use crate::models::File;
use crate::error::Result;
```

#### 1.3 错误处理
```rust
// 使用 ? 操作符传播错误
pub async fn process_file(path: &str) -> Result<FileInfo> {
    let metadata = tokio::fs::metadata(path).await?;
    let hash = calculate_file_hash(path).await?;

    Ok(FileInfo {
        path: path.to_string(),
        size: metadata.len(),
        hash,
    })
}

// 自定义错误类型
fn validate_file_path(path: &str) -> Result<()> {
    if !Path::new(path).exists() {
        return Err(AppError::FileNotFound(path.to_string()));
    }
    Ok(())
}
```

#### 1.4 异步代码
```rust
// 使用 async/await
pub async fn scan_directory(path: &str) -> Result<Vec<FileInfo>> {
    let mut entries = tokio::fs::read_dir(path).await?;
    let mut files = Vec::new();

    while let Some(entry) = entries.next_entry().await? {
        let path = entry.path();
        if path.is_file() {
            let file_info = process_file(&path.to_string_lossy()).await?;
            files.push(file_info);
        }
    }

    Ok(files)
}

// 使用 spawn 处理并发
pub async fn batch_process_files(paths: &[String]) -> Result<Vec<FileInfo>> {
    let tasks: Vec<_> = paths
        .iter()
        .map(|path| tokio::spawn(process_file(path)))
        .collect();

    let mut results = Vec::new();
    for task in tasks {
        match task.await {
            Ok(Ok(file_info)) => results.push(file_info),
            Ok(Err(e)) => return Err(e),
            Err(e) => return Err(AppError::TaskError(e.to_string())),
        }
    }

    Ok(results)
}
```

### 2. TypeScript 代码风格

#### 2.1 格式化
- **使用 Prettier**：统一代码格式
- **使用 ESLint**：代码质量检查
- **行长度**：不超过 100 个字符

#### 2.2 类型定义
```typescript
// 明确类型，避免使用 any
const fileList: FileInfo[] = [];

// 使用类型别名提高可读性
type FilePath = string;
type TagName = string;

// 使用联合类型
type SearchOperator = 'AND' | 'OR' | 'NOT';

// 使用可选属性和默认值
interface SearchOptions {
    tags?: string[];
    operator?: SearchOperator;
    limit?: number;
}

const defaultOptions: SearchOptions = {
    tags: [],
    operator: 'AND',
    limit: 100,
};
```

#### 2.3 React 组件规范
```typescript
// 使用函数组件和 Hooks
const FileList: React.FC<FileListProps> = ({ files, onSelect }) => {
    const [selectedIndex, setSelectedIndex] = useState<number | null>(null);
    const fileStore = useFileStore();

    // 使用 useCallback 优化性能
    const handleSelect = useCallback((index: number) => {
        setSelectedIndex(index);
        if (onSelect) {
            onSelect(files[index]);
        }
    }, [files, onSelect]);

    // 使用 useMemo 避免重复计算
    const filteredFiles = useMemo(() => {
        return files.filter(file => file.size > 0);
    }, [files]);

    return (
        <div className="file-list">
            {filteredFiles.map((file, index) => (
                <FileItem
                    key={file.path}
                    file={file}
                    isSelected={index === selectedIndex}
                    onSelect={() => handleSelect(index)}
                />
            ))}
        </div>
    );
};

// 导出组件
export default FileList;
```

#### 2.4 异步处理
```typescript
// 使用 async/await
async function loadFiles(directory: string): Promise<FileInfo[]> {
    try {
        const response = await invoke<FileInfo[]>('scan_directory', { path: directory });
        return response;
    } catch (error) {
        console.error('Failed to load files:', error);
        throw new Error(`无法加载目录: ${directory}`);
    }
}

// 在组件中使用
const FileBrowser: React.FC = () => {
    const [files, setFiles] = useState<FileInfo[]>([]);
    const [isLoading, setIsLoading] = useState(false);

    const loadDirectory = useCallback(async (directory: string) => {
        setIsLoading(true);
        try {
            const loadedFiles = await loadFiles(directory);
            setFiles(loadedFiles);
        } catch (error) {
            // 错误处理
        } finally {
            setIsLoading(false);
        }
    }, []);

    // ...
};
```

## 🧪 测试规范

### 1. Rust 测试规范

#### 1.1 单元测试
```rust
// 与被测试代码放在同一文件
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_calculate_file_hash() {
        // 准备测试数据
        let test_data = b"test content";
        let temp_file = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(temp_file.path(), test_data).unwrap();

        // 执行测试
        let result = calculate_file_hash(temp_file.path().to_str().unwrap());

        // 验证结果
        assert!(result.is_ok());
        let hash = result.unwrap();
        assert_eq!(hash.len(), 64); // SHA-256 哈希长度
    }

    #[tokio::test]
    async fn test_async_file_processing() {
        // 异步测试
        let result = process_file("test.txt").await;
        assert!(result.is_ok());
    }
}
```

#### 1.2 集成测试
```rust
// tests/integration_tests.rs
#[cfg(test)]
mod integration_tests {
    use file_manager::services::FileService;
    use file_manager::database::TestDatabase;

    #[tokio::test]
    async fn test_file_service_integration() {
        // 设置测试数据库
        let db = TestDatabase::new().await;
        let service = FileServiceImpl::new(db.pool());

        // 测试完整流程
        let result = service.add_tag_to_file("test.txt", "important").await;
        assert!(result.is_ok());
    }
}
```

### 2. TypeScript 测试规范

#### 2.1 组件测试
```typescript
// FileList.test.tsx
import { render, screen, fireEvent } from '@testing-library/react';
import FileList from './FileList';

const mockFiles: FileInfo[] = [
    { path: '/test/file1.txt', size: 1024, type: 'txt' },
    { path: '/test/file2.jpg', size: 2048, type: 'jpg' },
];

describe('FileList', () => {
    it('渲染文件列表', () => {
        render(<FileList files={mockFiles} />);

        expect(screen.getByText('file1.txt')).toBeInTheDocument();
        expect(screen.getByText('file2.jpg')).toBeInTheDocument();
    });

    it('处理文件选择', () => {
        const handleSelect = jest.fn();
        render(<FileList files={mockFiles} onSelect={handleSelect} />);

        fireEvent.click(screen.getByText('file1.txt'));
        expect(handleSelect).toHaveBeenCalledWith(mockFiles[0]);
    });
});
```

#### 2.2 工具函数测试
```typescript
// formatters.test.ts
import { formatFileSize } from './formatters';

describe('formatFileSize', () => {
    it('格式化字节', () => {
        expect(formatFileSize(0)).toBe('0 B');
        expect(formatFileSize(1023)).toBe('1023 B');
    });

    it('格式化千字节', () => {
        expect(formatFileSize(1024)).toBe('1 KB');
        expect(formatFileSize(2048)).toBe('2 KB');
    });

    it('格式化兆字节', () => {
        expect(formatFileSize(1048576)).toBe('1 MB');
    });
});
```

## 🔍 代码审查要点

### 1. Rust 代码审查
- [ ] **所有权和借用**：检查是否正确使用所有权系统
- [ ] **错误处理**：是否妥善处理所有可能的错误
- [ ] **异步安全**：检查 `Send` 和 `Sync` 约束
- [ ] **内存安全**：避免内存泄漏和悬垂指针
- [ ] **性能考虑**：避免不必要的克隆和分配

### 2. TypeScript 代码审查
- [ ] **类型安全**：避免使用 `any` 类型
- [ ] **React 最佳实践**：正确使用 Hooks
- [ ] **性能优化**：避免不必要的重渲染
- [ ] **错误处理**：妥善处理异步错误
- [ ] **可访问性**：考虑无障碍访问

## 🛠️ 工具配置

### 1. Rust 工具配置
```toml
# .cargo/config.toml
[build]
rustflags = ["-D", "warnings"]

[rust]
rustfmt = true
clippy = true
```

### 2. TypeScript 工具配置
```json
// .eslintrc.json
{
  "extends": [
    "eslint:recommended",
    "plugin:@typescript-eslint/recommended",
    "plugin:react/recommended",
    "plugin:react-hooks/recommended"
  ],
  "rules": {
    "@typescript-eslint/no-explicit-any": "error",
    "react/prop-types": "off"
  }
}
```

```json
// .prettierrc.json
{
  "semi": true,
  "trailingComma": "es5",
  "singleQuote": true,
  "printWidth": 100,
  "tabWidth": 2
}
```

## 📚 文档规范

### 1. API 文档
- **所有公共API必须包含文档注释**
- **包含示例代码**
- **说明错误情况**

### 2. README 文件
- **项目概述**：简要说明项目目的
- **快速开始**：安装和运行指南
- **架构说明**：技术栈和设计
- **开发指南**：如何贡献代码

### 3. CHANGELOG 文件
- **遵循语义化版本**：MAJOR.MINOR.PATCH
- **按版本分组**：记录所有变更
- **包含迁移指南**：重大变更说明

## 🚀 提交规范

### 1. Git 提交消息
```
<类型>: <简短描述>

<详细描述>

<相关issue>
```

**类型**：
- `feat`: 新功能
- `fix`: 修复bug
- `docs`: 文档更新
- `style`: 代码格式调整
- `refactor`: 代码重构
- `test`: 测试相关
- `chore`: 构建过程或辅助工具

**示例**：
```
feat: 添加文件标签管理功能

- 实现标签添加、删除、搜索功能
- 添加标签颜色支持
- 更新数据库迁移脚本

Closes #123
```

### 2. 分支命名
- `feature/`：新功能开发
- `bugfix/`：bug修复
- `hotfix/`：紧急修复
- `release/`：发布分支

**示例**：
```
feature/file-tag-management
bugfix/fix-memory-leak
hotfix/critical-security-fix
```

## 📊 代码质量指标

### 1. Rust 质量指标
- **测试覆盖率**：> 80%
- **Clippy 警告**：0 警告
- **编译时间**：优化增量编译
- **二进制大小**：控制最终可执行文件大小

### 2. TypeScript 质量指标
- **测试覆盖率**：> 80%
- **ESLint 错误**：0 错误
- **类型覆盖率**：100%
- **包大小**：控制 bundle 大小

---

*技术规范版本：v1.0*
*制定日期：2025-12-05*
*适用项目：文件管理系统（Rust + Tauri + TypeScript）*